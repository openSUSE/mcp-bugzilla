...
| `--tool-search` | `MCP_TOOL_SEARCH` | `False` | Collapse all tools into discovery + execute meta-tools. The LLM searches for relevant tools on demand (`search`), inspects their schemas (`get_schemas`), and chains calls via sandboxed Python code (`execute`). Requires `fastmcp[code-mode]`. Reduces context overhead when many tools are available |
